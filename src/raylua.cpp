...
// todo: a function to return a value from global(like a background color) or a
// default value preferably made in lua, if possible
...
int main() {
  ...
  luaL_openlibs(L);
  // load raylua.so and initialize the window with a width of 800 and height of
  // 600

  dostring(L, "raylua = require('raylua')");
  dostring(L, "raygui = require('lua_gui')");
  dostring(L, "luaShader = require('lua_shader')");
